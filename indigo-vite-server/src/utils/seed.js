...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/indigo-clone';
...
const generateFlights = () => {
  ...
  const flights = [];
  const departureTimes = [5, 6, 9, 13, 17, 21];
    ...
    for (let day = 0; day < 14; day++) {
      departureTimes.slice(0, 4).forEach((hour, hi) => {
        ...
        flights.push({
          ...
          seats: {
            total: 180,
            available: { economy: 120 + Math.floor(Math.random() * 30), business: 20 + Math.floor(Math.random() * 10) },
          },
          price: {
